...
export class VideoSeekSlider extends React.Component<Props, State> {
    ...
    private changeCurrentTimePosition(pageX: number): void {
        ...
        const time: number = +(percent * (this.props.max / 100)).toFixed(0);

        this.props.onChange(time, time + (this.props.offset ? this.props.offset : 0));
    }
    ...
    private getPositionStyle(time: number): object {
        const position: number = (time * 100) / this.props.max;
    ...
    private getThumbHandlerPosition(): object {
        const position: number = this.state.trackWidth / (this.props.max / this.props.currentTime);
    ...
    private getHoverTime(): string {
        ...
        const time: number = Math.floor(+(percent * (this.props.max / 100)));
    ...
    private drawHoverTime(): JSX.Element {
        const percent = (this.state.seekHoverPosition * 119) / this.state.trackWidth;
        const y = percent ? Math.floor(percent / 10) : 0;
        const x = percent ? Math.floor(percent % 10) : 0;
